...
- var 선언자의 특징
  변수 선언 - 데이터 타입 선언 X var, let, const
  - ES6이전부터 사용한 선언자
- 재선언, 재할당 가능 => 협업을 하면서 동일한 이름의 변수를 var로 선언할 경우 덮어쓰기가 가능해서 예상하지 않은 결과가 나올 수 있다.권장 X(덮어쓰기)
  - 전역변수에 사용
- **let** 선언자의 특징
- **const** 선언자의 특징
  ...
  var b = promps(); -> 7;
  console.log(a + b); -> 57;
  promps가 String만 입력되서 57이 나옴

  카멜표기법: 두 가지 이상의 단어가 합쳐질 경우, 두 번째부터 단어의 첫 글자를 대문자로
  var userName = "jj";
  스네이크 표기법: 드 개 이상의 단어가 합쳐질 경우 단어 사이에 \_ 삽입 (파이썬이 이렇게 씀)
  var user_name = "jj";
...
- 특징: 정수 또는 실수 모든 숫자
- 속성 및 함수
- 64비트 부동소수점 표현의 문제점: 자바스크립트는 자바등과 달리 메모리는 64비트 부동소수점 형태로 모든 숫자를 관리 => 문제(약간의 오차)가 발생함
- 실습(04_dataType.html)
